...
bool Poolingmask::InferShape(const std::vector<TEngine::TShape>& ishape, std::vector<TEngine::TShape>& oshape, int layout)
{
    ...
    oshape[0]=shape;// output data
    oshape[1]=shape;// output mask data
    return true;
}

float Poolingmask::GetFops(const std::vector<TShape>& inputs, const std::vector<TShape>& outputs)
{
...
void Poolingmask::SetSchema(void)
{
    Input({"input:float32"})
    ...
    .SetDoc(R"DOC(Poolingmask Layer)DOC");
}
