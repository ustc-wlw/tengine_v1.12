...
bool RefineOutput::InferShape(const std::vector<TEngine::TShape>& ishape, std::vector<TEngine::TShape>& oshape,
                                 int layout)
{
...
void RefineOutput::SetSchema(void)
{
    Input({"input:float32"})
        .Output({"output:float32"})
        .SetAttr("num_classes", 21)

        .SetDoc(R"DOC(RefineOutput Layer)DOC");
}
