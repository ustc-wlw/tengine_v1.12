...
{
    Tensor* input_tensor = node->GetInputTensor(0);
    Tensor* mask_tensor = node->GetInputTensor(1); // add by wanglw 20200428
    ...
    void* mask = ( void* )get_tensor_mem(mask_tensor); // add by wanglw 20200428
    ...
    int ret = kernel_run(input,mask,output,&op_param); // add by wanglw 20200428
    if(ret < 0)
        return false;
    return true;
}
