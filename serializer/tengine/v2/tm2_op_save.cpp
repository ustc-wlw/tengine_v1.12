...
{
    std::cout << "#### SaveTmUpsampleOp beagin.. " << std::endl;
    ...
    std::cout << "param scale = " << p->scale << std::endl;
    std::cout << "#### SaveTmUpsampleOp success #####\n" << std::endl;
    return WriteTmObject(start_ptr, cur_pos, &tm_op, sizeof(TM2_Operator));
}
...
tm_uoffset_t saveTmAxpyOp(void* const start_ptr, tm_uoffset_t* cur_pos, Operator* op){

    std::cout << "### saveTmAxpyOp beagin ..." << std::endl;
    AxpyParam* p = (dynamic_cast<Axpy*>(op))->GetParam();
    TM2_AxpyParam tm_param;
    tm_param.axis = p->axis;

    TM2_Operator tm_op;
    SetTmOperator(&tm_op, TM2_OPTYPE_AXPY, WriteTmObject(start_ptr, cur_pos, &tm_param, sizeof(TM2_AxpyParam)));
    // SetTmOperator(&tm_op, TM2_OPTYPE_AXPY,TM2_NOT_SET);
    tm_uoffset_t  ret = WriteTmObject(start_ptr, cur_pos, &tm_op, sizeof(TM2_Operator));
    std::cout << "### saveTmAxpyOp success ####\n" << std::endl;
    return ret;
}

// add by wanglw 20200427
tm_uoffset_t SaveTmPoolingmaskOp(void* const start_ptr, tm_uoffset_t* cur_pos, Operator* op)
{
    // std::cout << "### SaveTmPoolingmaskOp beagin ..." << std::endl;
    PoolmaskParam* p = (dynamic_cast<Poolingmask*>(op))->GetParam();
    TM2_PoolmaskParam tm_param;
    tm_param.alg = p->alg;
    tm_param.kernel_h = p->kernel_h;
    tm_param.kernel_w = p->kernel_w;
    tm_param.stride_h = p->stride_h;
    ...
    // std::cout << "kernel_h = " << p->kernel_h << ", kernel_w = " << p->kernel_w << std::endl;
    // std::cout << "stride_h = " << p->stride_h << ", stride_w = " << p->stride_w << std::endl;
    // std::cout << "pad_h = " << p->pad_h << ", pad_w = " << p->pad_w << std::endl;
    // std::cout << "global = " << p->global << ", alg = " << p->alg << std::endl;
    
    TM2_Operator tm_op;
    SetTmOperator(&tm_op, TM2_OPTYPE_POOLINGMASK, WriteTmObject(start_ptr, cur_pos, &tm_param, sizeof(TM2_PoolmaskParam)));
    // std::cout << "###  SaveTmPoolingmaskOp success #######\n" << std::endl;
    return WriteTmObject(start_ptr, cur_pos, &tm_op, sizeof(TM2_Operator));
}

// add by wanglw 20200508
tm_uoffset_t saveTmRefineOutputOp(void* const start_ptr, tm_uoffset_t* cur_pos, Operator* op)
{
    std::cout << "### saveTmRefineOutputOp beagin ..." << std::endl;
    RefineOutputParam* p = (dynamic_cast<RefineOutput*>(op))->GetParam();
    TM2_RefineOutputParam tm_param;
    tm_param.num_classes = p->num_classes;
    tm_param.keep_top_k = p->keep_top_k;
    tm_param.nms_top_k = p->nms_top_k;
    tm_param.confidence_threshold = p->confidence_threshold;
    tm_param.nms_threshold = p->nms_threshold;
    tm_param.objectness_threshold = p->objectness_score;

    TM2_Operator tm_op;
    SetTmOperator(&tm_op, TM2_OPTYPE_REFINEOUTPUT,
                  WriteTmObject(start_ptr, cur_pos, &tm_param, sizeof(TM2_RefineOutputParam)));
    std::cout << "###  saveTmRefineOutputOp success #######\n" << std::endl;
    return WriteTmObject(start_ptr, cur_pos, &tm_op, sizeof(TM2_Operator));
}
...
op_save_t SaveTmOpFunc(uint32_t op_type)
{
    switch(op_type)
    {
        ...
        case TM2_OPTYPE_AXPY:
            return saveTmAxpyOp; // add by wanglw 20200426
        case TM2_OPTYPE_POOLINGMASK:
            return SaveTmPoolingmaskOp; // add by wanglw 20200427
        case TM2_OPTYPE_REFINEOUTPUT:
            return saveTmRefineOutputOp; // add by wanglw 20200508
	default:
