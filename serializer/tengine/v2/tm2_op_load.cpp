...
bool LoadTmUpsampleOp(StaticGraph* graph, StaticNode* node, void* const start_ptr, const TM2_Operator* tm_op)
{
    ...
    // std::cout << "### LoadTmUpsampleOp beagin and op_str = " << op_str << std::endl;
    ...
    // std::cout << "### LoadTmUpsampleOp success ####\n" << std::endl;
    return true;
}
...
// add by wanglw 20200426
bool LoadTmAxpyOp(StaticGraph* graph, StaticNode* node, void* const start_ptr, const TM2_Operator* tm_op)
{
    std::cout << "### LoadTmAxpyOp beagin.." << std::endl;
    const std::string& op_str = TM2_OPSTR_AXPY;

    AxpyParam param = any_cast<AxpyParam>(OpManager::GetOpDefParam(op_str));
    const TM2_AxpyParam* tm_param = GetTmPtr<TM2_AxpyParam>(start_ptr, tm_op->offset_t_param);
    // param.axis = tm_param->axis;
    param.axis = 1;
    StaticOp* op = CreateStaticOp(graph, op_str);
    SetOperatorParam(op, param);
    ...
    std::cout << "### LoadTmAxpyOp success ####\n" << std::endl;
    return true;
}
// add by wanglw 20200427
bool LoadTmPoolingmaskOp(StaticGraph* graph, StaticNode* node, void* const start_ptr, const TM2_Operator* tm_op)
{
    // std::cout << "### LoadTmPoolingmaskOp beagin.." << std::endl;
    const std::string& op_str = TM2_OPSTR_POOLINGMASK;

    PoolmaskParam param = any_cast<PoolmaskParam>(OpManager::GetOpDefParam(op_str));
    const TM2_PoolmaskParam* tm_param = GetTmPtr<TM2_PoolmaskParam>(start_ptr, tm_op->offset_t_param);

    param.alg = static_cast<PoolmaskArg>(tm_param->alg);
    param.kernel_h = tm_param->kernel_h;
    param.kernel_w = tm_param->kernel_w;
    param.stride_h = tm_param->stride_h;
    param.stride_w = tm_param->stride_w;
    ...
    // std::cout << "kernel_h = " << tm_param->kernel_h << ", kernel_w = " << tm_param->kernel_w << std::endl;
    // std::cout << "stride_h = " << tm_param->stride_h << ", stride_w = " << tm_param->stride_w << std::endl;
    // std::cout << "pad_h = " << tm_param->pad_h << ", pad_w = " << tm_param->pad_w << std::endl;
    // std::cout << "global = " << tm_param->global << ", alg = " << param.alg << std::endl;
    
    param.kernel_shape.resize(2);

    param.kernel_shape[0]=tm_param->kernel_h;
    param.kernel_shape[1]=tm_param->kernel_w;

    param.strides.resize(2);
    param.strides[0]=tm_param->stride_h;
    param.strides[1]=tm_param->stride_w;

    param.pads.resize(4);
    param.pads[0]=tm_param->pad_h;
    param.pads[1]=tm_param->pad_w;
    param.pads[2]=tm_param->pad_h;
    param.pads[3]=tm_param->pad_w;

    StaticOp* op = CreateStaticOp(graph, op_str);
    SetOperatorParam(op, param);
    SetNodeOp(node, op);
    // std::cout << "### LoadTmPoolingmaskOp success #########\n" << std::endl;
    return true;
}

// add by wanglw 20200508
bool LoadTmRefineOutputOp(StaticGraph* graph, StaticNode* node, void* const start_ptr, const TM2_Operator* tm_op)
{
    std::cout << "### LoadTmRefineOutputOp beagin.." << std::endl;
    const std::string& op_str = TM2_OPSTR_REFINEOUTPUT;

    RefineOutputParam param = any_cast<RefineOutputParam>(OpManager::GetOpDefParam(op_str));
    const TM2_RefineOutputParam* tm_param = GetTmPtr<TM2_RefineOutputParam>(start_ptr, tm_op->offset_t_param);
    ...
    param.confidence_threshold = tm_param->confidence_threshold;
    param.nms_threshold = tm_param->nms_threshold;
    param.objectness_score = tm_param->objectness_threshold;

    std::cout << "num_classes = " << tm_param->num_classes << std::endl;
    std::cout << "keep_top_k = " << tm_param->keep_top_k  << std::endl;
    std::cout << "nms_top_k = " << tm_param->nms_top_k << std::endl;
    std::cout << "confidence_threshold = " << tm_param->confidence_threshold  << std::endl;
    std::cout << "nms_threshold = " << tm_param->nms_threshold << std::endl;
    std::cout << "objectness_score = " << tm_param->objectness_threshold  << std::endl;

    StaticOp* op = CreateStaticOp(graph, op_str);
    SetOperatorParam(op, param);
    SetNodeOp(node, op);
    std::cout << "### LoadTmRefineOutputOp success #########\n" << std::endl;
    return true;
...
op_load_t LoadTmOpFunc(uint32_t op_type)
{
    switch(op_type)
    {
        ...
        case TM2_OPTYPE_AXPY: // // add by wanglw 20200426
	        return LoadTmAxpyOp;
        case TM2_OPTYPE_POOLINGMASK:
            return LoadTmPoolingmaskOp;// add by wanglw 20200427
        case TM2_OPTYPE_REFINEOUTPUT:
            return LoadTmRefineOutputOp; // add by wanglw 20200508
...
std::string GetOpStr(uint32_t op_type)
{
    ...
    switch(op_type)
    {
        ...
        case TM2_OPTYPE_AXPY:
            return std::string(TM2_OPSTR_AXPY); // add by wanglw 20200426
        case TM2_OPTYPE_POOLINGMASK:
            return std::string(TM2_OPSTR_POOLINGMASK);// add by wanglw 20200427
        case TM2_OPTYPE_REFINEOUTPUT:
            return std::string(TM2_OPSTR_REFINEOUTPUT); // add by wanglw 20200508
	    default:
