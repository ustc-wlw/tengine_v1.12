...
#include "operator/axpy_param.hpp" // add by wanglw 20200426
#include "operator/poolingmask_param.hpp" // add by wanglw 20200427

#include "operator/refinedet_output_param.hpp" // add by wanglw 20200508
...
{
    std::cout << "## LoadCaffeUpsample beagin ...." << std::endl;
    ...
    std::cout << "upsample scale = " <<  param.scale << std::endl;
    std::cout << "## LoadCaffeUpsample success ###\n" << std::endl;
    return true;
}
...
static bool LoadCaffeAxpy(StaticGraph* graph, StaticNode* node, const te_caffe::LayerParameter& layer_param)
{
    std::cout << "## convert tool LoadCaffeAxpy begain.." << std::endl;
    const te_caffe::AxpyParameter& axpy_param=layer_param.axpy_param();

    AxpyParam param = any_cast<AxpyParam>(OpManager::GetOpDefParam("Axpy"));
    param.axis = axpy_param.axis();

    StaticOp* op = CreateStaticOp(graph, "Axpy");

    SetOperatorParam(op, param);

    SetNodeOp(node, op);

    std::cout << "## convert tool LoadCaffeAxpy sucess ###\n" << std::endl;
    return true;
}

// add by wanglw 20200427
static PoolmaskArg ConvertCaffePoolmask(te_caffe::PoolmaskParameter_PoolMethod method)
{
      if(method == te_caffe::PoolmaskParameter_PoolMethod_AVE)
           return kPoolmaskAvg; 
      else  if (method == te_caffe::PoolmaskParameter_PoolMethod_STOCHASTIC)
          return kPoolmaskRand;
...
static bool LoadCaffePoolingmask(StaticGraph* graph, StaticNode* node, const te_caffe::LayerParameter& layer_param)
{
    // std::cout << "## loadCaffePoolingmask beagin ...." << std::endl;
    const te_caffe::PoolmaskParameter& pool_param = layer_param.poolmask_param();

    PoolmaskParam param = any_cast<PoolmaskParam>(OpManager::GetOpDefParam("Poolingmask"));

    param.alg = ConvertCaffePoolmask(pool_param.pool());
    // std::cout << "alg type = " << param.alg << std::endl;
    if(pool_param.has_kernel_size())
    {
        param.kernel_h = pool_param.kernel_size();
        param.kernel_w = pool_param.kernel_size();
    ...
    StaticOp* op = CreateStaticOp(graph, "Poolingmask");
    ...
    // std::cout << "## loadCaffePoolingmask success ####\n" << std::endl;
    return true;
}

// add by wanglw 20200508
static bool  LoadCaffeRefineOutput(StaticGraph * graph, StaticNode * node, const te_caffe::LayerParameter& layer_param)
{
    std::cout << "## LoadCaffeRefineOutput beagin ...." << std::endl;
    RefineOutputParam  param=any_cast<RefineOutputParam>(OpManager::GetOpDefParam("RefineOutput"));

    const te_caffe::RefineOutputParameter& caffe_param=layer_param.refine_output_param();
    
    param.objectness_score=caffe_param.objectness_score();
    param.num_classes=caffe_param.num_classes();
    param.confidence_threshold=caffe_param.confidence_threshold();
    param.keep_top_k=caffe_param.keep_top_k();
    param.nms_threshold= caffe_param.nms_param().nms_threshold();
    ...
    StaticOp * op=CreateStaticOp(graph,"RefineOutput");
    SetOperatorParam(op,param);
    SetOperatorDynamicShape(op);
    SetNodeOp(node,op);

    std::cout << "## LoadCaffeRefineOutput success ####\n" << std::endl;
    return true;
...
bool CaffeSerializerRegisterOpLoader(void)
{
    ...
    // add by wanglw 20200426
    p_caffe->RegisterOpLoadMethod("Axpy", op_load_t(LoadCaffeAxpy));
    // add by wanglw 20200427
    p_caffe->RegisterOpLoadMethod("Poolingmask", op_load_t(LoadCaffePoolingmask));
    // add by wanglw 20200508
    p_caffe->RegisterOpLoadMethod("RefineOutput", op_load_t(LoadCaffeRefineOutput));
    ...
    // add by wanglw 20200426
    p_buddy->RegisterOpLoadMethod("Axpy", op_load_t(LoadCaffeAxpy));
    // add by wanglw 20200427
    p_buddy->RegisterOpLoadMethod("Poolingmask", op_load_t(LoadCaffePoolingmask));
    // add by wanglw 20200508
    p_buddy->RegisterOpLoadMethod("RefineOutput", op_load_t(LoadCaffeRefineOutput));
